...
489 tests are collected, 2 of them expected failures. A correct tree never reports a failure.
...
On a fresh clone with a game installed, expect roughly `Ran 486 ... OK (skipped=135, expected failures=2)` —
486 rather than 489 because three of them skip at class level, which unittest does not count in its total.
