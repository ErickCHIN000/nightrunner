...
- Tests: `.venv\Scripts\python.exe tests\run.py -q` (set `QT_QPA_PLATFORM=offscreen` first).
  429 tests collected, 2 expected failures, 0 failures on a correct tree. GUI tests skip without Qt; corpus tests
  ...
  published, so those always skip. A clone with the game present reports `Ran 426 ... OK (skipped=135,
  expected failures=2)` — 426 because three skip at class level, which unittest omits from its total.
