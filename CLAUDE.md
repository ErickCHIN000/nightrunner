...
- Tests: `.venv\Scripts\python.exe tests\run.py -q` (set `QT_QPA_PLATFORM=offscreen` first).
  394 tests collected, 2 expected failures, 0 failures on a correct tree. GUI tests skip without Qt; corpus tests
  ...
  published, so those always skip. A clone with the game present reports `Ran 391 ... OK (skipped=135,
  expected failures=2)` — 391 because three skip at class level, which unittest omits from its total.
