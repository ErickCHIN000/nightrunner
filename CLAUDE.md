...
- Tests: `.venv\Scripts\python.exe tests\run.py -q` (set `QT_QPA_PLATFORM=offscreen` first).
  ...
  published, so those always skip. A clone with the game present reports `Ran 486 ... OK (skipped=135,
  expected failures=2)` — 486 because three skip at class level, which unittest omits from its total.
