# Cast library by DTZxPorter (https://github.com/dtzxporter/cast), MIT licence — see CAST-LICENSE.txt.
# Vendored into nightrunner from DyingLightExplorer/dyinglightexplorer/vendor/cast.py (which pins upstream commit
# a8ca18a0acf3b97b19332c53b54b47fcc3217755). Faithful copy; the only nightrunner change is `Cast.load`, which used
# a DLE-private validation module and is replaced by a small standalone bounded loader (magic/version/root count).
# Node identifiers (little-endian fourcc): root 0x746F6F72 'root', modl 0x6C646F6D, mesh 0x6873656D, hair, blsh,
# skel 0x6C656B73, anim, curv, CMOV, ntif, bone 0x656E6F62, ikhd, cnst, matl 0x6C74616D, file 0x656C6966, colr,
# inst, meta 0x6174656D.
import struct
import itertools
...
class Cast(object):
    ...
    @staticmethod
    def load(path):
        """Load a cast file (nightrunner: standalone bounded loader replacing DLE's cast_validation hook)."""
        with open(path, "rb") as file:
            header = struct.unpack("<4I", file.read(16))
            if header[0] != 0x74736163:
                raise ValueError("not a cast file (bad magic 0x%08X)" % header[0])
            if header[1] != 1:
                raise ValueError("unsupported cast version %d" % header[1])
            if header[2] > 4096:
                raise ValueError("cast file declares %d root nodes" % header[2])
            cast = Cast()
            cast.rootNodes = [CastNode.load(file) for _ in range(header[2])]
        return cast

    def save(self, path):
        """Write the upstream node representation with an explicit file lifetime."""
        with open(path, "wb") as file:
            file.write(struct.pack("<4I", 0x74736163, 1, len(self.rootNodes), 0))
